...
// Component to initialize private space selection
function PrivateSpaceInitializer() {
  const { setActiveTab, setSelectedGroupId, selectedGroupId } = useSidebar();
  const { groups } = useGroups();

  useEffect(() => {
    // Only set if not already set and private group exists
  ...
  }, [groups, selectedGroupId, setSelectedGroupId, setActiveTab]);
...
function DashboardContent() {
  const { activeTab, setActiveTab, setSelectedGroupId } = useSidebar();
  const { user } = useUser();
  const { groups } = useGroups();
        ...
        >
          ...
          <div className="px-3 py-2.5">
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-md cursor-pointer transition-colors bg-primary hover:bg-primary/90 text-primary-foreground">
              ...
              <span className="text-sm font-medium flex-1">Upgrade to Pro</span>
